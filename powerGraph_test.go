...
// initPowerDataRecording loads the saved samples and spawns the 1 Hz recorder.
//
// The recorder is started with the shutdown flag already cleared so it
// observes it once and returns. Left running, it polls idleState every second
// for the remainder of the suite and races every later test that touches the
// display state. What is worth asserting about the load half is covered by
// TestLoadPowerData; this covers the spawn path without leaking a goroutine.
func TestInitPowerDataRecording(t *testing.T) {
	...
	// Long enough for the spawned goroutine to run its flag check and return.
