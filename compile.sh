...
env GOOS=linux CGO_ENABLED=1 CC=musl-gcc go build -o pcat2_mini_display_openwrt .
if [ $? -ne 0 ]; then
...
go build -o pcat2_mini_display_debian .
if [ $? -ne 0 ]; then
...
tar cvfJ pcat2_mini_display_package_$(date +%Y%m%d-%H%M)_${GIT_VERSION}.tar.xz "$PACKAGE_DIR"
