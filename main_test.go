...
func TestReturnFrameBuffer(t *testing.T) {
	...
	// The contract callers rely on is that a buffer handed out after a return
	// carries none of the previous frame's pixels — whether that comes from a
	// fresh allocation or a cleared pooled buffer is an implementation detail.
	// A fresh RGBA is fully zeroed (including alpha), so check for zero rather
	// than opaque black.
	if pixel := buf2.RGBAAt(10, 10); pixel != (color.RGBA{}) {
		t.Errorf("buffer handed out after return still has pixel data: %+v", pixel)
	}
}
...
func TestTimeConstants(t *testing.T) {
	...
	// 40 ms is a hardware-tuned value for the evdev key repeat rate — if this
	// fails, the constant was changed, not the test.
	if KEYBOARD_DEBOUNCE_TIME != 40*time.Millisecond {
		t.Errorf("KEYBOARD_DEBOUNCE_TIME should be 40ms, got %v", KEYBOARD_DEBOUNCE_TIME)
	}
...
// GetFrameBuffer must hand back a correctly sized buffer whether or not the
// bufferManager pool has been initialised — the nil-manager path is what runs
// in tests and during early startup before initBufferManager().
func TestGetFrameBufferWithoutManager(t *testing.T) {
	saved := bufferManager
	bufferManager = nil
	defer func() { bufferManager = saved }()

	buf := GetFrameBuffer(PCAT2_LCD_WIDTH, PCAT2_LCD_HEIGHT)
	if buf == nil {
		t.Fatal("GetFrameBuffer returned nil with no bufferManager")
	}
	if b := buf.Bounds(); b.Dx() != PCAT2_LCD_WIDTH || b.Dy() != PCAT2_LCD_HEIGHT {
		t.Errorf("got %dx%d, want %dx%d", b.Dx(), b.Dy(), PCAT2_LCD_WIDTH, PCAT2_LCD_HEIGHT)
	}

	// ReturnFrameBuffer must be a no-op rather than a panic when unpooled.
	ReturnFrameBuffer(buf)
	ReturnFrameBuffer(nil)
}

// init3FrameBuffers wires up the buffer manager, the render cache and both
// legacy framebuffer pairs. Every slot must come back non-nil and correctly
// sized, or the main loop dereferences a nil image on its first frame.
func TestInit3FrameBuffers(t *testing.T) {
	...
	if bufferManager == nil {
		t.Error("init3FrameBuffers left bufferManager nil")
	}
	if renderCache == nil {
		t.Error("init3FrameBuffers left renderCache nil")
	}
	if dirtyTracker == nil {
		t.Error("init3FrameBuffers left dirtyTracker nil")
	}
	...
	for _, tc := range []struct {
		...
		{"topBar", topBarFramebuffers, topBarFrameWidth, topBarFrameHeight},
		{"middle", middleFramebuffers, middleFrameWidth, middleFrameHeight},
		{"footer", footerFramebuffers, footerFrameWidth, footerFrameHeight},
	} {
		for i, buf := range tc.bufs {
			if buf == nil {
				t.Errorf("%s framebuffer %d is nil", tc.name, i)
				continue
			}
			if b := buf.Bounds(); b.Dx() != tc.w || b.Dy() != tc.h {
				t.Errorf("%s framebuffer %d is %dx%d, want %dx%d",
					tc.name, i, b.Dx(), b.Dy(), tc.w, tc.h)
			}
		}
	}

	// Both slots of a pair must be distinct images — the main loop
	// double-buffers by alternating between them.
	if topBarFramebuffers[0] == topBarFramebuffers[1] {
		t.Error("topBar double-buffer slots alias the same image")
	}
	if middleFramebuffers[0] == middleFramebuffers[1] {
		t.Error("middle double-buffer slots alias the same image")
	}
}
